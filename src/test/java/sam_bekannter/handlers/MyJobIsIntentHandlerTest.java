...
public class MyJobIsIntentHandlerTest {
    private MyJobIsIntentHandler handler;
    private final static String NAME = "Testuser";

    @Before
    public void setup() {
        handler = new MyJobIsIntentHandler();
    }
    ...
    public void testHandle() {
        ...
        final Response response = TestUtil.withInput("Maurer", PhrasesAndConstants.HOLIDAY_SLOT, handler, sessionAttributes, persistentAttributes);
        assertTrue(response.getOutputSpeech().toString().contains(PhrasesAndConstants.JOB_SAVED));
    }
}
