...
public class MyResidenceIsIntentHandlerTest {
    private MyResidenceIsIntentHandler handler;
    private final static String NAME = "Testuser";

    @Before
    public void setup() {
        handler = new MyResidenceIsIntentHandler();
    }
    ...
    public void testHandle() {
        ...
        final Response response = TestUtil.withInput("Berlin", PhrasesAndConstants.RESIDENCE_SLOT, handler, sessionAttributes, persistentAttributes);
        assertTrue(response.getOutputSpeech().toString().contains(PhrasesAndConstants.RESIDENCE_SAVED));
    }
}
